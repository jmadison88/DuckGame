...
class GameScene: SKScene {
    ...
    func createDucks() {
        ...
        duck.zPosition = 0
        addChild(duck)
        let move = SKAction.move(to: CGPoint(x: 100, y: 100), duration: 2.0)
        let move2 = SKAction.move(to: CGPoint(x: 0, y: -100), duration: 2.0)
        let move3 = SKAction.move(to: CGPoint(x: -100, y: -100), duration: 2.0)
        let wait = SKAction.wait(forDuration: 1.0)
        let reset = SKAction.move(to: CGPoint(x: 0.0, y: 0.0), duration: 1.0)
        let changeTexture = SKAction.run {
            let newTexture = SKTexture(imageNamed: "OriginalSwimming")
